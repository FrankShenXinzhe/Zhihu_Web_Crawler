...
# 提取文章链接
# 定义文件路径
path = r"C:\Users\86158\Desktop\原始文章.csv"

# 提取文件内容至 data
data = pd.read_csv(path, encoding='utf-8')

# 将文件第一列（列名为‘url’）写入列表
article_url_list = data['url'].values.tolist()


# 逐一访问 article_url_list 中的每个链接
for article_url in article_url_list:
    ...
    # 获取问题
    article = wd.find_element(By.CSS_SELECTOR, '.Post-Title').get_attribute('innerText')

    # 去除问题中的非中文字符
    article = remove_non_chinese(article)

    # 写入模块
    # 创建txt文件（以问题命名）
    f = open(fr'C:\Users\86158\Desktop\文章内容汇总\文章{article}.txt', 'w', encoding='utf-8')
